...
while epoch < num_epochs:
    ...
    with torch.no_grad():
        ...
        with open('../Log_out_files/original_and_reconstructed_LN(baseline)_CSL.txt', 'a') as file:
    ...
    if epoch % 5 == 0:
        ...
        ax.legend()
        fig.savefig('../plots/elbo_plot_LN(baseline)_CSL.png')
        plt.close(fig)
        ...
        ax.legend()
        fig.savefig('../plots/kl_plot_LN(baseline)_CSL.png')
        plt.close(fig)
        ...
        ax.legend()
        fig.savefig('../plots/nll_plot_LN(baseline)_CSL.png')
        plt.close(fig)
        ...
        ax.legend()
        fig.savefig('../plots/loss_plot_LN(baseline)_CSL.png')
        plt.close(fig)
...
vae_path = "../VAE_settings/vae_settings_LN(baseline)_CSL.pth"
encoder_path = "../VAE_settings/encoder_LN(baseline)_CSL.pth"
decoder_path = "../VAE_settings/decoder_LN(baseline)_CSL.pth"
...
fig, ax = plt.subplots()
ax.set_title(r'ELBO: $\mathcal{L} ( \mathbf{x} )$')
ax.plot(training_data['elbo'][5:], label='Training')
ax.plot(validation_data['elbo'][5:], label='Validation')
ax.legend()
fig.savefig('../plots/elbo_plot_LN(baseline)_CSL.png')
plt.close(fig)
...
fig, ax = plt.subplots()
ax.set_title(r'$\log p_\theta(\mathbf{x} | \mathbf{z})$')
ax.plot(training_data['log_px'][5:], label='Training')
ax.plot(validation_data['log_px'][5:], label='Validation')
ax.legend()
fig.savefig('../plots/nll_plot_LN(baseline)_CSL.png')
plt.close(fig)
