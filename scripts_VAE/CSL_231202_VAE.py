...
class VariationalAutoencoder(nn.Module):
    ...
    * a Gaussian posterior `q_\phi(z|x) = N(z | \mu(x), \sigma(x))`
    ...
    def observation_model(self, z:Tensor) -> Distribution:
        ...
        return Normal(px_params, 1.0)  # Assuming unit variance
...
archs4_train_dataloader = DataLoader(archs4_train, batch_size=train_batch_size, shuffle=True, collate_fn=DynamicNormalizeTransform())
...
gtex_test_dataloader = DataLoader(gtex_test, batch_size=eval_batch_size, shuffle=True, collate_fn=DynamicNormalizeTransform())
...
while epoch < num_epochs:
    ...
    with torch.no_grad():
        ...
        x = next(iter(gtex_test_dataloader))
        x = x.to(device)
        ...
        reconstructed = vae(x)['px'].rsample().view(-1, *vae.input_shape).cpu().numpy()

        # Print and save original and reconstructed data to a text file
        with open('../Log_out_files/original_and_reconstructed_CSL.txt', 'a') as file:
    ...
    if epoch % 5 == 0:
        ...
        ax.legend()
        fig.savefig('../plots/elbo_plot_CSL.png')
        plt.close(fig)
        ...
        ax.legend()
        fig.savefig('../plots/kl_plot_CSL.png')
        plt.close(fig)
        ...
        ax.legend()
        fig.savefig('../plots/nll_plot_CSL.png')
        plt.close(fig)
        ...
        ax.legend()
        fig.savefig('../plots/loss_plot_CSL.png')
        plt.close(fig)
...
vae_path = "../VAE_settings/vae_settings_CSL.pth"
encoder_path = "../VAE_settings/encoder_CSL.pth"
decoder_path = "../VAE_settings/decoder_CSL.pth"
