...
# Feed-forward Neural network architecture (original input size)
class RegressionModel_complete(nn.Module):
    def __init__(self, input_size, hidden_size_1, hidden_size_2, output_size, dropout_rate=0):
        super(RegressionModel_complete, self).__init__()
...
for fold, (train_indices, test_indices) in enumerate(stratified_kfold.split(np.zeros(len(tissue_labels_og)), tissue_labels_og)):
    ...
    model_og = RegressionModel_complete(input_size_og, 256, 32, output_size, 0)
